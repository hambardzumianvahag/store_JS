...
register.addEventListener('click', function () {
	if (pass1.value !== pass2.value) {
		...
		result.innerHTML = 'пароль должен содержать не менее 6 символов'
		pass2.after(result)
	}
	else if (!uppercasePattern.test(pass1.value)) {
		result.innerHTML = 'пароль должен содержать заглавную букву'
		pass2.after(result)

	}
	else if (!emailPattern.test(email.value)) {
		result.innerHTML = 'Неправильный адрес электронной почты'
		pass2.after(result)
	}
	else {
		result.innerHTML = 'Вы успешно зарегистрировались на нашем сайте'
		pass2.after(result)
	}
...
function generateProducts(products) {
	...
	products.forEach((item) => {
	   ...
	   <p>$${item.price}</p> 
	   <div class='btn_div'>
	   <button data-count='0' class='fav_btn'>добавить в избранное</button>
		<button data-count='0' class="add_btn">добавить в корзину</button>
		</div>
		`
		root.append(docelems)
	})
}
generateProducts(every)
...
let category = document.querySelector('.category_inp')
...
function getCategory() {
	...
	categories.forEach(elem => {
		...
		ctg.innerHTML = `<input class='checkb' id=${elem} value=${elem} type='checkbox'/>
...
checkboxes.forEach((checkbox) => {
	checkbox.addEventListener('change', function () {
		if (this.checked) {
			...
			array = array.filter((value) => value !== this.value);
			favBasket()
			itemBasket()
		}
		...
		if (array.length === 0) {
			...
			filteredProducts = every.filter((item) => array.includes(item.category));
...
rangeMin.addEventListener('input', compareInputValues)
rangeMax.addEventListener('input', compareInputValues)


function compareInputValues() {
	const value1 = parseFloat(min.value);
	const value2 = parseFloat(max.value);
	filteredProducts = every.filter(item => item.price > value1 && item.price < value2)
	generateProducts(filteredProducts)
	favBasket()
	itemBasket()
}
...
sort.addEventListener('click', (e) => {
	if (e.target.value == 'high') {
		let sorted = every.sort((a, b) => b.price - a.price)
		generateProducts(sorted)
		favBasket()
		itemBasket()
	}
	else if (e.target.value == 'low') {
		let sorted = every.sort((a, b) => a.price - b.price)
		generateProducts(sorted)
		favBasket()
		itemBasket()
	}
	else if (e.target.value == 'rating') {
		let sorted = every.sort((a, b) => b.rating - a.rating)
		generateProducts(sorted)
		favBasket()
		itemBasket()
	}
...
function favBasket() {
	let clickCount
	let add_fav = document.getElementsByClassName("fav_btn");
	for (const item of add_fav) {
		item.addEventListener("click", (e) => {
			clickCount = parseInt(item.getAttribute("data-count"));
			clickCount++
			item.setAttribute('data-count', clickCount)
...
function itemBasket() {
	...
	add_btn.forEach((item, index) => {
		item.addEventListener("click", (e) => {
			...
			sum += +every[index].price;
			new_btn2.innerHTML = "удалить из корзины";
